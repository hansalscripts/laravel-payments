...
class PaymobPayment extends BaseController implements PaymentInterface
{
    ...
    {
        $this->paymob_api_key = config('laravel-payments.PAYMOB_API_KEY');
        $this->paymob_integration_id = config('laravel-payments.PAYMOB_INTEGRATION_ID');
        $this->paymob_iframe_id = config("laravel-payments.PAYMOB_IFRAME_ID");
    ...
    public function verify(Request $request): array
    {
        ...
        if ( hash_equals(hash_hmac('sha512', $string, config('laravel-payments.PAYMOB_HMAC')),$request['hmac']) ){
            if ($request['success'] == "true") {
                ...
                return [
                    'success' => false,
                    'payment_id'=>$request['order'],
                    'message' => __('laravel-payments::messages.PAYMENT_FAILED_WITH_CODE',['CODE'=>$this->getErrorMessage($request['txn_response_code'])]),
                    'process_data' => $request->all()
                ];
            }
    ...
    public function getErrorMessage($code){
        $errors=[
            'BLOCKED'=>__('laravel-payments::messages.Process_Has_Been_Blocked_From_System'),
            'B'=>__('laravel-payments::messages.Process_Has_Been_Blocked_From_System'),
            '5'=>__('laravel-payments::messages.Balance_is_not_enough'),
            'F'=>__('laravel-payments::messages.Your_card_is_not_authorized_with_3D_secure'),
            '7'=>__('laravel-payments::messages.Incorrect_card_expiration_date'),
            '2'=>__('laravel-payments::messages.Declined'),
            '6051'=>__('laravel-payments::messages.Balance_is_not_enough'),
            '637'=>__('laravel-payments::messages.The_OTP_number_was_entered_incorrectly'),
            '11'=>__('laravel-payments::messages.Security_checks_are_not_passed_by_the_system'),
        ];
        if(isset($errors[$code]))
            return $errors[$code];
        else
            return __('laravel-payments::messages.An_error_occurred_while_executing_the_operation');
    }
