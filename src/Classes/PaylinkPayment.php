...
class PaylinkPayment extends BaseController implements PaymentInterface
{
    ...
    {
        $this->paylink_api_key = config('laravel-payments.PAYLINK_API_KEY');
        $this->paylink_app_id = config('laravel-payments.PAYLINK_APP_ID');
        $this->paylink_mode = config('laravel-payments.PAYLINK_MODE');
        ...
        if(isset($get_token_id['id_token'])){
            ...
            if(isset($response['orderStatus']) &&  $response['orderStatus'] == "Paid"){
                ...
                return [
                    'success' => false,
                    'payment_id'=>$request['transactionNo'],
                    'message' => __('laravel-payments::messages.PAYMENT_FAILED_WITH_CODE'),
                    'process_data' => $response
                ];
            }
        }
