...
class PayPalCreditPayment extends BaseController implements PaymentInterface
{
    ...
    {
        $this->paypal_credit_client_id = config('laravel-payments.PAYPAL_CREDIT_CLIENT_ID');
        $this->paypal_credit_secret = config('laravel-payments.PAYPAL_CREDIT_SECRET');
        $this->verify_route_name = config('laravel-payments.VERIFY_ROUTE_NAME');
        $this->paypal_credit_mode = config('laravel-payments.PAYPAL_CREDIT_MODE');
        $this->currency = config('laravel-payments.PAYPAL_CREDIT_CURRENCY');
    }
    ...
    public function pay($amount = null, $user_id = null, $user_first_name = null, $user_last_name = null, $user_email = null, $user_phone = null, $source = null)
    {
        ...
        try {
            $fetch_address = Http::withoutVerifying()
                ->get('http://ip-api.com/json/' . $this->get_ip())
                ->json();
    ...
    {
        return view('laravel-payments::html.paypal-credit', ['data' => $data])->render();
    }
    ...
    public function get_ip()
    {
        ...
        if ($ipaddress == "127.0.0.1") {
            $ip = Http::withoutVerifying()->get('https://api.ipify.org/?format=json')
